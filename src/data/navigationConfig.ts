...
// Поиск соответствующего объекта по строке роута
// Ищем точное совпадение в корневых элементах
// Если не нашли — ищем в первом уровне `submenu`
...
// Получить список подкатегорий раздела
// По названию раздела (title) находит его в navTree
// и возвращает его массив submenu
...
// Хлебные крошки для заданного пути
...
// найти раздел по ключу-части пути.
export const getCategoryByKey = (key: string) => navTree.find((cat) => cat.route === '/' + key);

// по массиву сегментов URL (разделённому через location.split('/')) вернуть наиболее «глубокую» найденную категорию или подкатегорию.
...
// разобрать полный URL (строку location) и вернуть объект:
